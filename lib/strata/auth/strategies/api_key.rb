...
module Strata
  module Auth
    module Strategies
      ...
      # It validates the key from the X-API-Key header using SHA-256 with a salt
      # and constant-time comparison to prevent timing attacks.
      class ApiKey < Base
        def initialize(salt:, api_key_digest:)
          @salt = salt
        ...
        def authenticate!(request)
          ...
          computed = Digest::SHA256.hexdigest("#{@salt}#{provided_key}")
